# Contribute to Continuous Engineering CLI
Want to hack on the Continuous Engineering CLI? Awesome! This page contains information that will help you set up a development environment for working on the source code.
...
### Create a new branch
First, ensure that your local repository is up-to-date with the latest version of ce-cli. More details on [GitHub help](https://help.github.com/articles/syncing-a-fork/).
...
### Build your change
With the prerequisites installed and your fork of Continuous Engineering CLI cloned, you can make changes to local Continuous Engineering CLI source code and hack as much as you want.

Run `make` to build the ce binaries:
...
This is a bit more advanced but required to ensure a proper Git history of Continuous Engineering CLI. Git allows you to [rebase](https://git-scm.com/docs/git-rebase) commits. In other words: it allows you to rewrite the commit history.
...
```sh
pick 911c35b Add "How to contribute to Continuous Engineering CLI" tutorial
pick 33c8973 Begin workflow
pick 3502f2e Refactoring and typo fixes
```

In the case above we should merge the last 2 commits in the commit of this tutorial (`Add "How to contribute to Continuous Engineering CLI" tutorial`). You can "squash" commits, i.e. merge two or more commits into a single one.
...
```sh
reword 911c35b Add "How to contribute to Continuous Engineering CLI" tutorial
squash 33c8973 Begin workflow
squash 3502f2e Refactoring and typo fixes
```
...
Continuous Engineering CLI enforces the DCO using the a [bot](https://github.com/probot/dco). You can view the details on the DCO check by viewing the `Checks` tab in the GitHub pull request.
...
Continuous Engineering CLI uses [conventional commits](https://www.conventionalcommits.org/en/v1.0.0/) as it's commit message format. These are particularly important as semantic releases are in use, and they use the commit messages to determine the type of changes in the codebase. Following formalized conventions for commit messages the semantic release automatically determines the next [semantic version](https://semver.org) number and generates a changelog based on the conventional commit.

Semantic releases originate in the [Angular Commit Message Conventions](https://github.com/angular/angular.js/blob/master/DEVELOPERS.md#-git-commit-guidelines), and the rules described there are the ones used by Continuous Engineering CLI
...
### Open a pull request
We made a lot of progress. Good work. In this step we finally open a pull request to submit our additions. Open the [Continuous Engineering CLI master repository](https://github.com/continuousengineeringproject/ce-cli/) on GitHub in your browser.
...
Then the maintainers will review your PR, potentially initiate discussion around your change and finally, merge it successfully in Continuous Engineering CLI. Congratulations !
...
In the example you can see that we pass the generator to use: `pegomock generate` the package path name: `github.com/continuousengineeringproject/ce-cli/pkg/util` the name of the interface: `CommandInterface` and finally an output directive to write the generated file to a mock sub-folder. To keep things nice and tidy it's best to write each mocked interface to a separate file in this folder. So in this case: `-o mocks/command_interface.go`
...
import (
	...
	"github.com/continuousengineeringproject/ce-cli/pkg/util"
	mocks "github.com/continuousengineeringproject/ce-cli/pkg/util/mocks"
	. "github.com/petergtz/pegomock"
	"github.com/stretchr/testify/assert"
)

func TestCEBinaryLocationSuccess(t *testing.T) {
	t.Parallel()
	commandInterface := mocks.NewMockCommandInterface()
	When(commandInterface.RunWithoutRetry()).ThenReturn("/test/something/bin/ce", nil)
